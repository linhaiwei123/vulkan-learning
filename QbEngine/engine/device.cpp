...
void qb::Device::init(App* app) {
	...
	/*std::vector<VkFormatFeatureFlagBits> testFlags = {
	...
	for (VkFormat testFormat : {VK_FORMAT_R8_UINT}) {
		VkFormatProperties props;
		vkGetPhysicalDeviceFormatProperties(physical, testFormat, &props);
		for (VkFormatFeatureFlagBits flag : testFlags) {
			auto linear = props.linearTilingFeatures & flag;
			auto optimal = props.optimalTilingFeatures & flag;
			auto buffer = props.bufferFeatures & flag;
			bool debug;
		}
	}*/
}
